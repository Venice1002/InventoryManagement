...
public class WorkerManager
{
	...
	public void modifyWorker(Worker worker) throws BusinessException
	{
		Connection conn = null;
		try
		{
			...
			rs.close();
			pst.close();
			sql = "update Worker set workerName = ?, workerDepartment=?, workerPermission = ?, workerPhone = ? where workerId=?";
			pst = conn.prepareStatement(sql);
			pst.setString(1, worker.getWorkerName());
			pst.setString(2, worker.getWorkerDepartment());
			pst.setInt(3, worker.getWorkerPermission());
			pst.setString(4, worker.getWorkerPhone());
			pst.setInt(5, worker.getWorkerId());
			pst.execute();
			pst.close();
		}
		catch (SQLException e)
